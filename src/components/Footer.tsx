...
export const Footer = ({ isCalculating = false, productsCount = 0 }: FooterProps) => {
  ...
  return (
    <motion.div 
      className={`fixed bottom-0 left-0 right-0 bg-slate-800 border-t border-slate-600 shadow-lg z-50 transition-all duration-300 safe-bottom footer-fixed ${isCalculating ? 'opacity-30 pointer-events-none' : ''}`}
      initial={{ y: 0 }}
      animate={{ y: 0 }}
      layoutId="footer"
      layout
