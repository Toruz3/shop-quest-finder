        ...
        <div className="flex-1 overflow-hidden flex flex-col p-4 pt-3 space-y-3">
          ...
          <div className="flex items-center gap-2">
            <div className="relative flex-1">
              <Input value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder={`Cerca tra ${list.items.length} prodotti...`} className="h-8 pl-8 text-sm bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-600 rounded-lg" />
          ...
          <div className="flex-1 overflow-y-auto">
            {filteredItems.length > 0 ? <div className="space-y-1.5">
                ...
                delay: index * 0.02
                      ...
                      <Button size="icon" variant="ghost" onClick={() => handleRemoveProduct(item)} className="h-6 w-6 opacity-0 group-hover:opacity-100 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-all duration-150 flex-shrink-0">
                        <X size={12} />
                      </Button>
                    </motion.div>)}
                </AnimatePresence>
