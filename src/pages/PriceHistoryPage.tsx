      ...
      <div className="container px-3 py-4 relative z-10">
        <div className="max-w-md mx-auto">
          ...
          <div className="relative mb-4">
            <Input placeholder="Cerca prodotto" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pr-10 py-5 bg-white border-primary-100 focus:border-primary-300 focus:ring focus:ring-primary-200 transition-all duration-300 rounded-xl shadow-sm hover:shadow-md focus:shadow-md w-full" />
          ...
          <Tabs defaultValue="tracked" className="mb-4" onValueChange={setSelectedTab}>
            <TabsList className="w-full grid grid-cols-3 h-12 rounded-lg p-1 bg-primary-50">
              <TabsTrigger value="tracked" className="rounded-md data-[state=active]:text-primary data-[state=active]:shadow-sm transition-all">
                Monitorati
              </TabsTrigger>
              <TabsTrigger value="offers" className="rounded-md data-[state=active]:text-primary data-[state=active]:shadow-sm transition-all">
                In offerta
              </TabsTrigger>
              <TabsTrigger value="trends" className="rounded-md data-[state=active]:text-primary data-[state=active]:shadow-sm transition-all">
                Tendenze
              </TabsTrigger>
            </TabsList>
          </Tabs>
          ...
          <Card className="p-4 mb-4 border border-neutral-200 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              ...
              <Badge variant="outline" className={`${selectedProductData?.trend === "down" ? "text-green-600 bg-green-50" : selectedProductData?.trend === "up" ? "text-red-600 bg-red-50" : "text-blue-600 bg-blue-50"}`}>
            ...
            <div className="h-56 w-full">
              <ResponsiveContainer width="100%" height="100%">
                ...
                bottom: 5
                  ...
                  <YAxis tick={{
                  fontSize: 10
                }} axisLine={false} tickLine={false} domain={['dataMin - 0.1', 'dataMax + 0.1']} tickFormatter={value => `€${value.toFixed(2)}`} />
                  <Tooltip formatter={value => [`€${Number(value).toFixed(2)}`, ""]} labelFormatter={label => `Settimana: ${label}`} />
                  <Line type="monotone" dataKey="Esselunga" stroke="#2ecc71" strokeWidth={2} activeDot={{
                  r: 6
                }} />
                  <Line type="monotone" dataKey="Conad" stroke="#8a6cff" strokeWidth={2} activeDot={{
                  r: 6
                }} />
                  <Line type="monotone" dataKey="Carrefour" stroke="#3498db" strokeWidth={2} activeDot={{
                  r: 6
                }} />
          ...
          <div className="space-y-2">
            {filteredProducts.filter(product => (selectedTab === "offers" ? product.discount !== null : true) && (selectedTab === "trends" ? product.trend !== "stable" : true)).map(product => <Card key={product.id} className={`p-3 border ${selectedProduct === product.id ? "border-primary-300 bg-primary-50/50" : "border-neutral-200 hover:border-primary-200"} transition-all shadow-sm hover:shadow-md`} onClick={() => setSelectedProduct(product.id)}>
                  <div className="flex justify-between items-center">
                    <div>
                      <h3 className="font-medium text-neutral-800 text-left">{product.name}</h3>
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <Badge variant="outline" className={`text-xs ${product.trend === "down" ? "text-green-600 bg-green-50 border-green-200" : product.trend === "up" ? "text-red-600 bg-red-50 border-red-200" : "text-blue-600 bg-blue-50 border-blue-200"}`}>
