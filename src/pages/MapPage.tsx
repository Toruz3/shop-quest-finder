...
const MapPage = () => {
  ...
  return <motion.div className="min-h-screen w-full flex flex-col overflow-hidden main-content" initial={{
    opacity: 0
  }} animate={{
    opacity: 1
  }} exit={{
          ...
          <div className="w-full max-w-md mx-auto">
            ...
            <div className="space-y-4">
              <div className="relative">
                <Input placeholder="Cerca supermercato per nome" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pr-10 py-5 bg-white border-primary-100 focus:border-primary-300 focus:ring focus:ring-primary-200 transition-all duration-300 rounded-xl shadow-sm hover:shadow-md focus:shadow-md w-full" />
              ...
              <Tabs defaultValue="nearby" className="w-full" onValueChange={setSelectedTab}>
                <TabsList className="w-full grid grid-cols-3 h-12 rounded-lg p-1 bg-primary-50">
                  <TabsTrigger value="nearby" className="rounded-md data-[state=active]:text-primary data-[state=active]:shadow-sm transition-all">
                    Più vicini
                  </TabsTrigger>
                  <TabsTrigger value="cheapest" className="rounded-md data-[state=active]:text-primary data-[state=active]:shadow-sm transition-all">
                    Più economici
                  </TabsTrigger>
              ...
              <div>
                <Button variant="outline" size="sm" className="w-full justify-between bg-white" onClick={() => setShowFilters(!showFilters)}>
              ...
              <div className="relative rounded-xl overflow-hidden h-48 bg-neutral-100">
                {!isMapLoaded ? <div className="absolute inset-0 flex items-center justify-center">
                    ...
                    <img src="https://images.unsplash.com/photo-1545065053-73e294de55ed?auto=format&fit=crop&w=600&h=200&q=80" alt="Map" className="w-full h-full object-cover" />
              ...
              <div className="space-y-3">
                {stores.filter(store => (selectedTab === "offers" ? store.hasOffers : true) && (searchTerm ? store.name.toLowerCase().includes(searchTerm.toLowerCase()) : true)).sort((a, b) => {
                ...
                return 0;
              }).map(store => <Card key={store.id} className="p-3 border border-neutral-200 hover:border-primary-200 transition-all shadow-sm hover:shadow-md">
                      ...
                      {store.hasOffers && <div className="mt-2 pt-2 border-t border-dashed border-neutral-200">
                          <p className="text-xs text-accent-600 font-medium">Offerte speciali:</p>
                          <ul className="mt-1 space-y-1">
                            {store.specialOffers.map((offer, idx) => <li key={idx} className="text-xs text-neutral-700 flex items-start gap-1">
